...
const CONFIG = {
    ...
    SUPABASE_KEY: 'eyJhbGciOiJICTCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImhhbXF5YW56Z2Z6Y3hueG5xemV2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDc4Mjg2NDAsImV4cCI6MjA2MzQwNDY0MH0.6l3dW3OXC8M_CX2TrejJR8EY5xgZvsIcKzTIXQ14rTs',
    ADMIN_PASSWORD: '1907'
};
...
function renderClothes() {
    ...
    roupasFiltradas.forEach(roupa => {
        ...
        // --- CORREÇÃO AQUI ---
        card.innerHTML = `<img src="${roupa.imagem_url}" class="roupa-image" alt="${roupa.nome}" onclick="showQuickView(${roupa.id})">
            ${wishlistButtonHtml}
            <div class="roupa-info">
                <h3 class="roupa-nome">${roupa.nome}</h3>
                <p class="roupa-descricao">${roupa.descricao || ''}</p>
...
async function handleRoupaSubmit(e) {
    ...
    // --- CORREÇÃO AQUI ---
    const roupaData = { nome: document.getElementById('roupaNome').value, descricao: document.getElementById('roupaDescricao').value, tamanho: document.getElementById('roupaTamanho').value, preco: parseFloat(document.getElementById('roupaPreco').value), categoria: document.getElementById('roupaCategoria').value, imagem_url: document.getElementById('roupaImagem').value };
    let res;
    if (id) { res = await supabaseClient.from('roupas').update(roupaData).eq('id', id); }
    else { res = await supabaseClient.from('roupas').insert([roupaData]); }
    if (res.error) { showToast("Erro ao salvar roupa: " + res.error.message, "error"); }
    else { showToast('Roupa salva com sucesso!', 'success'); hideModal('roupaModal'); await loadRoupasFromDB(); renderClothes(); }
}
...
function showEditRoupaModal(id) {
    const r = roupas.find(rp => rp.id === id);
    if (!r) return;
    document.getElementById('roupaModalTitle').textContent = 'Editar Roupa';
    document.getElementById('roupaId').value = r.id;
    document.getElementById('roupaNome').value = r.nome;
    document.getElementById('roupaDescricao').value = r.descricao;
    document.getElementById('roupaTamanho').value = r.tamanho;
    document.getElementById('roupaPreco').value = r.preco;
    document.getElementById('roupaCategoria').value = r.categoria;
    // --- CORREÇÃO AQUI ---
    document.getElementById('roupaImagem').value = r.imagem_url;
...
function showQuickView(id) {
    ...
    document.getElementById('quickViewContent').innerHTML = `
        <img src="${r.imagem_url}" alt="${r.nome}" class="quick-view-image">
        <div class="quick-view-details">
            <p>${r.descricao || `Cor: ${r.cor}`}</p>
            <p><strong>Tamanho:</strong> ${r.tamanho}</p>
            <h3 class="roupa-preco">R$ ${parseFloat(r.preco).toFixed(2)}</h3>
