...
function renderClothes() {
    ...
    roupas.forEach(roupa => {
        ...
        card.innerHTML = `<img src="${roupa.imagem_url}" class="roupa-image" alt="${roupa.nome}" onclick="showQuickView(${roupa.id})">${wishlistButtonHtml}<div class="roupa-info"><h3 class="roupa-nome">${roupa.nome}</h3><p class="roupa-descricao">Cor: ${roupa.cor || 'Não informada'}</p><div class="roupa-details"><span class="roupa-tamanho">${roupa.tamanho}</span><span class="roupa-preco">R$ ${parseFloat(roupa.preco).toFixed(2)}</span></div><div class="roupa-actions">${currentUser && currentUser.isAdmin ? adminButtonsHtml : clientButtonHtml}</div></div>`;
...
async function handleAdminLogin(e) {
    ...
    if (password === CONFIG.ADMIN_PASSWORD) {
        ...
        if (userReservations.length > 0) {
            const reservedIds = userReservations.map(res => res.roupa.id);
            const { error } = await supabaseClient.from('roupas').update({ status: 'disponivel' }).in('id', reservedIds);
            if (error) {
                showToast("Erro ao limpar reservas antigas.", "error");
            } else {
                reservations = reservations.filter(res => res.userEmail !== userEmail);
                saveReservations();
        ...
        await loadData();
        updateAndRenderAll();
        showToast(`Privilégios de admin concedidos!`, 'success');
    } else {
...
async function handleRoupaSubmit(e) { e.preventDefault(); const id = document.getElementById('roupaId').value; const roupaData = { nome: document.getElementById('roupaNome').value, cor: document.getElementById('roupaCor').value, tamanho: document.getElementById('roupaTamanho').value, preco: parseFloat(document.getElementById('roupaPreco').value), imagem_url: document.getElementById('roupaImagem').value }; let response; if (id) { response = await supabaseClient.from('roupas').update(roupaData).eq('id', id); } else { response = await supabaseClient.from('roupas').insert([roupaData]); } if (response.error) { console.error("Erro ao salvar roupa:", response.error); showToast("Erro ao salvar roupa.", "error"); } else { showToast('Roupa salva com sucesso!', 'success'); hideModal('roupaModal'); await loadData(); renderClothes(); } }
async function deleteRoupa(roupaId) { const { error } = await supabaseClient.from('roupas').delete().eq('id', roupaId); if (error) { console.error("Erro ao excluir roupa:", error); showToast("Erro ao excluir roupa.", "error"); } else { showToast("Roupa excluída com sucesso!", "success"); await loadData(); renderClothes(); } }
...
function showQuickView(id) { const r = roupas.find(rp => rp.id === id); if (!r) return; document.getElementById('quickViewNome').textContent = r.nome; document.getElementById('quickViewContent').innerHTML = `<img src="${r.imagem_url}" alt="${r.nome}" class="quick-view-image"><div class="quick-view-details"><p><b>Cor:</b> ${r.cor}</p><p><strong>Tamanho:</strong> ${r.tamanho}</p><h3 class="roupa-preco">R$ ${parseFloat(r.preco).toFixed(2)}</h3><div class="roupa-actions">${r.status === 'disponivel' && !(currentUser && currentUser.isAdmin) ? `<button class="btn btn-primary" onclick="addFromQuickView(${r.id})"><i class="fas fa-shopping-cart"></i> Reservar</button>` : `<button class="btn btn-reserved" disabled><i class="fas fa-bookmark"></i> Indisponível</button>`}</div></div>`; showModal('quickViewModal'); }
...
async function checkout() {
    ...
    showToast("Processando suas reservas...", "info");
    const roupasIdsParaReservar = cart.map(item => item.id);
    const { error } = await supabaseClient.from('roupas').update({ status: 'reservado' }).in('id', roupasIdsParaReservar);

    if (error) {
        console.error("Erro ao reservar itens:", error);
        showToast("Ocorreu um erro ao finalizar a reserva.", "error");
        return;
    }
    ...
    saveCart();
    saveReservations();
    showToast(`${itemsCount} peça(s) reservada(s) com sucesso!`, 'success');
...
async function cancelReservation(resId) {
    ...
    const { error } = await supabaseClient.from('roupas').update({ status: 'disponivel' }).eq('id', roupaId);
