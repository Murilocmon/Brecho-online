...
// ======================================================
async function handleRoupaSubmit(e) {e.preventDefault(); const id = document.getElementById('roupaId').value; const roupaData = { nome: document.getElementById('roupaNome').value, descricao: document.getElementById('roupaDescricao').value, tamanho: document.getElementById('roupaTamanho').value, preco: parseFloat(document.getElementById('roupaPreco').value), categoria: document.getElementById('roupaCategoria').value, imagem: document.getElementById('roupaImagem').value }; let res; if (id) { res = await supabaseClient.from('roupas').update(roupaData).eq('id', id); } else { res = await supabaseClient.from('roupas').insert([roupaData]); } if (res.error) { showToast("Erro ao salvar roupa.", "error"); } else { showToast('Roupa salva com sucesso!', 'success'); hideModal('roupaModal'); await loadRoupasFromDB(); renderClothes(); } }
