...
async function checkUserSession() {
    ...
    if (session) {
        ...
        const { data: profile, error } = await supabaseClient
            .from('perfis') // Usa a nova tabela 'perfis'
...
// ======================================================
// EVENT LISTENERS E UI (Sem alterações)
// ======================================================
...
async function handleRegister(e) {
    ...
    if (data.user) {
        ...
        const { error: profileError } = await supabaseClient
            .from('perfis') // Usa a nova tabela 'perfis'
            .insert({ id: data.user.id, name: name, wishlist: [] });

        if (profileError) {
            showToast(`Erro ao criar perfil: ${profileError.message}`, 'error');
        } else {
...
async function toggleWishlist(id, e) {
    ...
    if (error) {
        ...
        if (itemIndex > -1) currentWishlist.push(id); else currentWishlist.pop();
    } else {
...
// Funções de CRUD de Roupas, Carrinho, etc. (sem alterações)
