...
const CONFIG = {
    ...
    ADMIN_PASSWORD: '1907',
    RESERVATION_DAYS_LIMIT: 3
};
const STORAGE_KEYS = { CART: 'achadinhos_cart' };
...
async function checkUserSession() {
    ...
    if (session && session.user) {
        ...
        if (profile) {
            currentUser = { id: session.user.id, email: session.user.email, name: profile.name, wishlist: profile.wishlist || [], avatar_url: profile.avatar_url, isAdmin: false };
        }
    } else {
        currentUser = null;
    }
...
async function loadDataFromDB() {
    showToast("Atualizando a loja...", "info");
    await checkExpiredReservations(); // Verifica e limpa reservas expiradas
    ...
    const [roupasResponse, reservationsResponse] = await Promise.all([
        ...
        supabaseClient.from('reservas').select('*, roupas(id, nome, imagem_url)')
    ]);

    roupas = roupasResponse.data || [];
...
function updateStatusFromReservations() {
    ...
    roupas.forEach(roupa => {
        roupa.status = reservedIds.includes(roupa.id) ? 'reservado' : 'disponivel';
    });
}

// ======================================================
// EVENT LISTENERS E UI (Seu código completo)
// ======================================================
...
function renderDashboard() { const d = document.getElementById('adminDashboard'); d.innerHTML = `<div class="stat-card"><h4>Itens na Loja</h4><p>${roupas.filter(r => r.status === 'disponivel').length}</p></div><div class="stat-card"><h4>Itens Reservados</h4><p>${reservations.length}</p></div>`; renderAdminReservations(); }

// ======================================================
// AUTENTICAÇÃO, ADMIN E PERFIL
// ======================================================
...
async function handlePasswordResetRequest(e) { e.preventDefault(); const email = document.getElementById('resetEmail').value; showToast("Enviando link...", "info"); const { error } = await supabaseClient.auth.resetPasswordForEmail(email, { redirectTo: window.location.href }); if (error) { showToast(`Erro: ${error.message}`, "error"); } else { showToast("Se o email estiver cadastrado, um link de recuperação foi enviado!", "success"); hideModal('resetPasswordModal'); } }
...
function renderAdminReservations() { const container = document.getElementById('adminReservationsList'); container.innerHTML = ''; if (reservations.length === 0) { container.innerHTML = '<p>Nenhuma reserva ativa.</p>'; return; } reservations.forEach(res => { const roupa = roupas.find(r => r.id === res.roupa_id); if (!roupa) return; const item = document.createElement('div'); item.className = 'reservation-item'; item.innerHTML = `<img src="${roupa.imagem}" class="reservation-item-image" alt="${roupa.nome}"><div class="reservation-item-info"><div class="reservation-item-name">${roupa.nome}</div><div class="reservation-item-date">Reservado por: ${res.user_email}</div></div><button class="btn btn-danger btn-small" onclick="cancelReservation(${res.id})"><i class="fas fa-times"></i> Cancelar</button>`; container.appendChild(item); }); }
...
async function cancelReservation(resId) {
    ...
    await loadData();
    if (!document.getElementById('profileModal').classList.contains('hidden')) { renderMyReservations(); }
    renderClothes();
}
function renderMyReservations() { const c = document.getElementById('profileReservations'); c.innerHTML = ''; if (!currentUser) return; const myRes = reservations.filter(r => r.user_id === currentUser.id); if (myRes.length === 0) { c.innerHTML = '<p>Você não tem reservas ativas.</p>'; return; } myRes.forEach(r => { const roupa = roupas.find(rp => rp.id === r.roupa_id); if(!roupa) return; const i = document.createElement('div'); i.className = 'reservation-item'; const d = new Date(r.created_at).toLocaleDateString('pt-BR'); i.innerHTML = `<img src="${roupa.imagem}" class="reservation-item-image"><div class="reservation-item-info"><div class="reservation-item-name">${roupa.nome}</div><div class="reservation-item-date">Reservado em: ${d}</div></div><button class="btn btn-danger btn-small" onclick="cancelReservation(${r.id})"><i class="fas fa-times"></i></button>`; c.appendChild(i); }); }
